...
router.get('/states', (req, res) => {
    if (indianLocations.length === 0) {
        return res.json(FALLBACK_STATES.map(s => ({ id: s, code: s, name: s })));
    }
    const states = indianLocations.map(s => ({ id: s.state, code: s.code || s.state, name: s.state }));
    res.json(states);
});
...
router.get('/districts', (req, res) => {
    ...
    const districts = state.districts.map(d => ({ id: d.district, code: d.district, name: d.district }));
    res.json(districts);
});
...
router.get('/talukas', (req, res) => {
    ...
    for (const state of indianLocations) {
        ...
        if (district) {
            subDistricts = (district.subDistricts || []).map(sd => ({ id: sd.subDistrict, code: sd.subDistrict, name: sd.subDistrict }));
            break;
        }
    }
    ...
    for (const state of indianLocations) {
        for (const district of state.districts) {
            ...
            if (subDist && subDist.villages) {
                villages = subDist.villages.map(v => ({ id: v, code: v, name: v }));
                break;
            }
        }
